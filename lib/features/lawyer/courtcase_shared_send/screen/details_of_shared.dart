...
import 'package:ataaby/features/lawyer/courtcase_shared_send/cubit/state.dart';
...
class DetailsOfSharedCourtcase extends StatefulWidget {
  const DetailsOfSharedCourtcase({super.key, required this.model});
  final CourtCaseDetailsSharedAndSendModel model;
  @override
  State<DetailsOfSharedCourtcase> createState() =>
      _DetailsOfSharedCourtcaseState();
}

class _DetailsOfSharedCourtcaseState extends State<DetailsOfSharedCourtcase> {
  @override
  void initState() {
    context
        .read<CourtcaseSharedandSendCubit>()
        .courtCaseContributionRequestById(widget.model.id.toString() ?? '');
    super.initState();
  }
                              ...
                              children: [
                                ...
                                Padding(
                                    ...
                                    child: HeaderOfCourtCaseWidget(
                                        isShared: true,
                                        model: cubit
                                            .getAllCourtCaseSharedModelDetails
                                            ?.data)),
                                10.h.verticalSpace,
  ...
  Widget _buildUpdatesSection(CourtcaseSharedandSendCubit cubit) {
    final updates =
        cubit.getAllCourtCaseSharedModelDetails?.data?.courtCase?.updates;
  ...
  Widget _buildDetailedCaseView(CourtcaseSharedandSendCubit cubit) {
    return Column(
      children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          Text(
                            cubit.getAllCourtCaseSharedModelDetails?.data
                          ...
                          Row(
                            ...
                            children: [
                              ///// const rate //////
                              Text(
                                "${cubit.getAllCourtCaseSharedModelDetails?.data?.courtCase?.client?.rates.toStringAsFixed(2) ?? '0'} (${cubit.getAllCourtCaseSharedModelDetails?.data?.courtCase?.client?.ratesCount ?? '0'})",
                                style: getRegularStyle(
                                  color: AppColors.blackLight,
                                  fontSize: 14.sp * textScaleFactor(context),
                                ),
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  cubit.getAllCourtCaseSharedModelDetails?.data?.courtCase
                          ?.details ??
                      '',
                  style: getBoldStyle(
                      fontSize: 14.sp * textScaleFactor(context),
        ...
        (cubit.getAllCourtCaseSharedModelDetails?.data?.courtCase?.files
                    ?.length ==
                0)
            ? Container()
            : Padding(
